...
def lru_cache(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if cache_instance is None:
            ...
            store = MemoryStore()
            cache_instance = Cache(store=store)
...
def smart_list_cache(
    ...
    id_extractor: Optional[Callable] = (
        lambda x: x.id if hasattr(x, "id") else x.get("id")
    ),  # Function to extract ids from results
) -> Callable:
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if list_arg_position < len(args) and isinstance(
                args[list_arg_position], list
            ):
                ...
                for item_id in id_list:
                    ...
                    args_copy = list(args)
                    args_copy[list_arg_position] = [
                        item_id
                    ]  # Wrap in list to maintain expected structure
                ...
                print(
                    f"Partial cache hit. Fetching {len(uncached_ids)}/{len(id_list)} ids from provider."
                )
